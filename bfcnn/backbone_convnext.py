...
from .backbone_blocks import convnext_blocks_full
...
def builder(
        ...
        kernel_size: int,
        filters: int,
        block_kernels: List[int] = [7, 1, 1],
        block_filters: List[int] = [96, 384, 96],
        block_groups: List[int] = [1, 1, 1],
        block_depthwise: List[int] = [1, -1, -1],
        block_regularizer: List[str] = None,
        block_activation: List[str] = ["linear", "gelu", "linear"],
    ...
    """
    # --- logging
    logger.info("building convnext backbone")
